...
export class GuidedTourComponent implements AfterViewInit, OnInit, OnDestroy {
  ...
  private updateTourSteps(url: string = '', forceStepUpdate: boolean = false): void {
    ...
    if (url.includes('/home?')) {
      ...
      this.tourType = 'home';
      this.tooltipText = 'Tour the Trendall collection search page';
      currentSteps = this.searchHomeSteps;
    } else if (url.includes('/search?')) {
      if(url.includes('isNLS=true')) { 
    ...
    return [
      ...
      },
      {
        ...
        popover: {
          ...
          description: "<p>The main menu lets you start a new search of the Trendall collection and you can also view information about the Trendall Centre.</p><p>Note that when you're in 'mobile' view, some options that are usually on the page (e.g. the 'Advanced search') are within this menu instead.</p>",
          side: "bottom",
          align: "end",
          popoverClass: 'ltu-tour ltu-tour-wide'
        }
   ...
   if(scope.isSmallView || scope.isMobileView) {
    ...
    steps.push(
      ...
      },
      {
        ...
        popover: {
          title: "Search form",
          description: "If you didn't get the results that you were after, try a new search. You can always add more search parameters by modifying the generated query.",
          side: "bottom",
          align: "center",
          onPrevClick: function(element: any, step: any, options: any): void {
            if(scope.isSmallView || scope.isMobileView) {
